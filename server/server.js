...
var GridFSConnector = require('grid_fs_conn_ssrx');
...
var upload = multer({ dest: 'uploads/' }).single('fax2.json');
  ...
  try {
    upload(req, res, function (err) {
      if (err) {
        ...
        var fax_json = fs.readFileSync(__dirname+'/../'+req.file.path, 'utf8');
        
        localGridFSConnector.uploadFax(JSON.parse(fax_json));
      }
    });
...
app.use('/download_fax', async function (req, res, next) {
  try {
    localGridFSConnector.downloadFax("519f7ec226983e6a48614677fa2ec25a1567585998872.tiff", res);//change filename here
  } catch (err) {
    console.error(err);
    res.status(500).send('Internal Server Error');
  }
...
app.use('/download_metadata', async function (req, res, next) {
  try {
    localGridFSConnector.getFaxMetadata("519f7ec226983e6a48614677fa2ec25a1567585998872.tiff", res);
  } catch (err) {
    console.error(err);
    res.status(500).send('Internal Server Error');
  }
...
app.use('/query_fax', async function (req, res, next) {
  try {
    localGridFSConnector.searchFax({ "metadata.container": "ssrx" }, res);
  } catch (err) {
    console.error(err);
    res.status(500).send('Internal Server Error');
  }
